...
class ObjectTableTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $ot = new ObjectTable();
        $ot->add(new ClassItem('file1', 1, 'class1'));
        $ot->add(new ClassItem('file1', 1, 'class2'));
        $ot->add(new ClassItem('file1', 1, 'class3'));
        ...
        foreach ($ot as $key => $val) {
            ...
            $this->assertEquals(new ClassItem('file1', 1, 'class' . $idx), $val);
            $idx += 1;
        }
    }
}
